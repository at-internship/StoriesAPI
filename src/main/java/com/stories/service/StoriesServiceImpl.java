...
public class StoriesServiceImpl implements StoriesService {
	...
	public String createTask(TasksDomain taskDomain, String id) throws Exception {
		...
		if (storiesRepository.existsById(id)) {
			if (!StringUtils.isEmpty(taskDomain.getName())) {
				if (userNullTaskValidation(taskDomain.getAssignee())) {
					if (statusTaskValidation(StoriesApiConstants.statusArray, taskDomain.getStatus())) {
						...
						logger.debug(
								"Creating task for the US: " + storyModel.get_id() + " with the json: " + taskModel);
						storiesRepository.save(storyModel);
						return taskModel.get_id();
					} else {
						throw new EntityNotFoundException(StoriesApiConstants.storyFieldStatusInvalidException,
								StoriesApiConstants.httpCodeStatusBadRequest, StoriesApiConstants.pathStories);
					}
				} else {
					throw new EntityNotFoundException(StoriesApiConstants.taskFieldAssigneeNotFoundException,
			...
			throw new EntityNotFoundException(StoriesApiConstants.storyFieldIdNotFoundException,
					StoriesApiConstants.pathStories + id + StoriesApiConstants.pathTasks);
		}
	}
	...
	public void deleteStory(String id) throws Exception {
		if (!storiesRepository.existsById(id)) {
			throw new EntityNotFoundException(StoriesApiConstants.storyFieldIdNotFoundException,
					StoriesApiConstants.pathStories + id);
		} else
	...
	public void deleteTask(String id, String taskId) throws Exception {
		if (!storiesRepository.existsById(id)) {
			...
			if (!tasks.isEmpty()) {
				...
				throw new EntityNotFoundException(StoriesApiConstants.storyFieldTasksEmptyException,
	...
	public List<StoryDomain> getAllStories() throws Exception {
		...
		if (allStoriesModel == null)
			throw new EntityNotFoundException(StoriesApiConstants.storiesNotFoundException,
					StoriesApiConstants.pathStories);
	...
	private StoryModel nameValidation(StoryModel storyModel) throws EntityNotFoundException {
		try {
			...
			throw new EntityNotFoundException(StoriesApiConstants.storyFieldNameExistException, HttpStatus.CONFLICT,
					StoriesApiConstants.pathStories);
		}
	}
		...
		for (int i = 0; i < validateSpecialCharacterField.size(); i++) {
			...
			if (!err) {
				...
				if (StringUtils.isEmpty(validationRespons)) {
					...
					validationRespons = validationRespons + " AND "
							+ StoriesApiConstants.ValidationStorySpecialCharacterMessage[i];
				}
			}
		}
	...
	private boolean userNullTaskValidation(String assigneeId) throws EntityNotFoundException {
		if (StringUtils.isEmpty(assigneeId)) {
			...
			if (!userClient.existUserById(assigneeId))
				throw new EntityNotFoundException(StoriesApiConstants.taskFieldAssigneeNotFoundException,
						HttpStatus.CONFLICT, StoriesApiConstants.pathStories);
			return true;
		}
	}
		...
		for (int i = 0; i < validateSpecialCharacterField.size(); i++) {
			...
			if (!err) {
				...
				if (StringUtils.isEmpty(validationRespons)) {
					...
					validationRespons = validationRespons + " AND "
							+ StoriesApiConstants.ValidationTaskSpecialCharacterMessage[i];
				}
			}
		}
